...
const EditClient = () => {
  const { id } = useParams();
  const { clientName, Adresse, email, mobile, matricule } = useLoaderData();
  ...
  const handleClientUpdate = (event) => {
    ...
    const clientUpdatedObj = { clientName, Adresse, email, mobile, matricule };
    alert(mobile);
    fetch(`http://localhost:3050/update-client/${id}`, {
      method: "PATCH",
      headers: {
        "Content-type": "application/json",
      },
      body: JSON.stringify(clientUpdatedObj),
    }).then((res) =>
      res.json().then((data) => {
        alert("Client modifié avec succés");
        form.reset();
        navigate("/produits");
      })
    );
      ...
      <div className="w-full lg:w-8/12 px-4 mx-auto mt-6">
        <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
          ...
          <div className="flex-auto px-4 lg:px-10 py-10 pt-0 bg-gray-100		">
            <form
              className="flex flex-col flex-wrap lg:w[1180px] gap-4"
